...
class VerticalSpeed:
    ...
    def __smooth_distance_value(self, newDF, distance_column_name):
        ...
        newDF['distance_shift1_0'] = shifted1
        newDF['distance_shift0_4'] = shifted2
        newDF['distance_shift0_7'] = shifted3

        if Configurations(self.__folderStruct).is_debug():
            columns_y = [distance_column_name, "distance_shift1_0", "distance_shift0_7", "distance_shift0_4"]
            self.__save_graphs_smooth_distance(newDF, distance_column_name, columns_y, 4000, 5000)
