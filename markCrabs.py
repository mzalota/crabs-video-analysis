    ...
    if filename.endswith(".jpg"):
        ...
        for crab in foundCrabs:
            crabsDF = crabsDF.append(
                {'dir': framesDir, 'filename': filename, 'frameID': frameID, 'crabNumber': crabNumber, 'crabWidthPixels': crab.diagonal(),
                 'crabLocationX': crab.centerPoint().x, 'crabLocationY': crab.centerPoint().y, 'crabWidthLine': crab},
                ignore_index=True)

            print("crab", framesDir, filename, frameID, crabNumber, crab.diagonal(), crab.centerPoint().x, crab.centerPoint().y,
