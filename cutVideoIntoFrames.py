...
imageWin2 = ImageWindow("topSubimage", Point(0, 0))


videoStream = VideoStream('C:/workspaces/AnjutkaVideo/Kara_Sea_Crab_Video_st_5993_2018/V3__R_20180915_205551.avi')


def processFrame(frameNumber, prevFrameNumber):
    print 'Read a new frame: ', frameNumber
    windowName = 'Detected_' + str(frameNumber)
    ...
    try:
        ...
        image = videoStream.readImageObj(frameNumber)
        imageWin.showWindowAndWait(image.asNumpyArray(), 1000)

        if prevFrameNumber > 0:
            prevImage = videoStream.readImageObj(prevFrameNumber)

            boxLine = Box(Point(0, prevImage.height() - 3), Point(prevImage.width(), prevImage.height()))
            prevImage.drawBoxOnImage(boxLine)

            box = Box(Point(0,prevImage.height()-200), Point(prevImage.width(),prevImage.height()))
            subImage = prevImage.subImage(box)

            subImage.drawTextInBox(Box(Point(0,0), Point(80,50)), prevFrameNumber)

            imageWin2.showWindowAndWaitForClick(subImage.asNumpyArray())
...
with open(framesFilePath) as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            frameNumber = row[0]
            print (frameNumber)
            processFrame(frameNumber, prevFrameNumber)
            line_count += 1
